...
public class MethodRef {
     ...
     */
        ...
        System.out.println("abs1 : " + abs1.apply(-50));

        // 2) 람다식 변환
        Function<Integer, Integer> abs2 = num -> Math.abs(num);
        System.out.println("abs2 : " + abs2.apply(-50));

        // 3) 메서드 참조 방식으로 변환
        // == 람다식 num -> Math.abs(num)과 같은 모양, 같은 역할의 메서드
        // int Math.abs(int num)
        Function<Integer, Integer> abs3 = Math::abs;
        System.out.println("abs3 : " + abs3.apply(-50));
        ...
        // 리스트 순차 접근 후 출력
        // List.of() : 값을 바로 넣어서 수정 불가능한(immutable) 리스트를 만드는 방법
...
*/
        ...
        // 4. 정적 메서드 참조로 변환
        System.out.println("sum04 : "+ sumTwoNumber(Integer::sum, 10, 20));
    }
