package ch09_collection_generic.collection.set.ex;

import java.util.*;

public class SetEx {
    // 컬렉션 특징 : 객체만 저장, 크기제한X, 다양한 기능 제공
    ...
    public void ex1(){
        ...
        // 사용 조건 2) : 저장되는 객체에  equals() 오버라이딩 필수

        // String, Integer 등 Java 제공 객체는
        // 기본적으로 hashCode(), equlas() 등이 오버라이딩 돼있음

       Set<String> set = new HashSet<String>();

       // 1. boolean set.add(E e) : 추가 E: 요소
       set.add("딸기");
       set.add("포도");
       set.add("오렌지");
       set.add("체리");
        ...
        //    (반환형)
        // - set에 저장된 객체 중 e와 똑같은 객체가 있으면 true 반환,
        //   없으면 false 반환
        ...
        // - set에 저장된 객체가 없다면(비어있다면) ture, 아니면 false 반환

        System.out.println("set이 " +(set.isEmpty() ? "빔" : "안빔"));
        System.out.println("set이 " +(set.size() == 0    ? "빔" : "안빔"));
    }
    public void ex2(){
        // Set에 저장된 값 꺼내쓰기
        ...
        // set의 내용을 순차(반복) 접근할수 있는 형태로 변경해서 반환
