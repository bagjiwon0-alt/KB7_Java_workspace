...
public class Car {
    ...
    public Car(String model, String color, int maxSpeed){
        ...
        // this.model : 현재객체(디스)의 모델값
        // 현재 객체가 가지고있는 필드 model(this.model)에 매개변수 model 값을 대입
        this.color = color;// 필드 = 매개변수
        this.maxSpeed = maxSpeed;// 필드 = 매개변수
    ...
    // ** 오버로딩 (Over Loading) ** //
    // - 클래스내에서 동일한 이름의 *메소드(생성자 포함)*를 여러개 작성하는 기법
    //      -> 하나의 이름으로 여러기능을 수행할 수 있게 해준다.

    // 오버로딩 조건
    //      1) 메소드(생성자포함)의 이름이 동일
    //      2) 매개변수의 개수, 타입, 순서 중 한개라도 달라야 함.

    // 매개변수의 개수와 타입이 같지만 순서가 다름. -> 오버로딩 성립
    public Car(String model, int maxSpeed, String color){
//        System.out.println("this 생성자");
        // (주의) this()는(=this생성자) 반드시 첫 줄에 작성돼야한다.
        this(model, maxSpeed);

        // this 생성자-> this(): 현재 객체의 다른 생성자를 호출
        // this() 사용 이유 : 중복코드제거, 코드길이감소, 재사용성 증가
        // (가독성이 안좋아져서 많이 사용되진 않음)
...
//    public Car(String brand,int minSpeed){}
    // 매개변수명은 다르지만 개수, 타입, 순서가 동일하기 때문에 에러 발생(오버로딩 불가!!!)
    // -> 매개변수명은 중요하지 않아!!
